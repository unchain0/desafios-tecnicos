...
<li
    class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0"
    wire:key="{{ $task->id }}"
>
    <div class="flex items-center gap-3 flex-1 min-w-0">
        <span class="text-xl" aria-label="{{ $task->done ? 'Tarefa concluída' : 'Tarefa pendente' }}">
            {{ $task->done ? '✅' : '⬜' }}
        </span>
    ...
    <div class="ml-4 shrink-0 flex gap-2">
        ...
        <button
            @click="$dispatch('confirm-delete', { action: { method: 'deleteTask', params: {{ $task->id }} } })"
            wire:loading.attr="disabled"
